...
var LineItemQuantity = class extends HTMLElement {
  constructor() {
    ...
    // Attach event listeners for quantity changes and link clicks
...
onQuantityChanged_fn = function(event, target) {
  // Trigger quantity change when the user modifies the input field
...
onChangeLinkClicked_fn = function(event, target) {
  ...
  // Handle quantity changes via link clicks (used in cart line items)
...
changeLineItemQuantity_fn = async function(lineKey, targetQuantity) {
  if (window.themeVariables.settings.pageType === "cart") {
    // If we're on the cart page, redirect to update quantity via URL (Impulse cart mechanism)
    window.location.href = `${Shopify.routes.root}cart/change?id=${lineKey}&quantity=${targetQuantity}`;
  } else {
    // If we're not on the cart page, handle AJAX quantity updates
    ...
    // Prepare bundled sections for the cart (Impulse-specific)
    let sectionsToBundle = [];
    document.documentElement.dispatchEvent(new CustomEvent("cart:prepare-bundled-sections", { bubbles: true, detail: { sections: sectionsToBundle } }));

    // Make an AJAX request to update cart quantity
    ...
    // Update line item quantity in the cart after change
    ...
    // Dispatch a global cart change event
...
// Register the custom element if it hasn't been defined
